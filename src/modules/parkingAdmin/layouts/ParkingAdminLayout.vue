...
<template>
  ...
  <div class="flex w-screen h-screen text-gray-700">
    ...
    >
      ...
      <!-- Sidebar Logo -->
      <a
        class="flex items-center justify-center flex-shrink-0 w-full h-32 bg-white"
        href="#"
      >
        <img
      ...
      <div class="flex flex-col flex-grow p-4 overflow-auto">
        ...
        <RouterLink
          class="flex items-center flex-shrink-0 h-10 px-2 text-sm font-medium rounded hover:bg-gray-300"
          :to="`/parking-admin/parking/${authStore.user?.id_parking_lot}`"
        >
          <FontAwesomeIcon :icon="faCar" class="mr-2 text-blue-500" />
          <span v-if="isSidebarOpen" class="leading-none">Mi parqueadero</span>
        </RouterLink>
        <RouterLink
          class="flex items-center flex-shrink-0 h-10 px-2 text-sm font-medium rounded hover:bg-gray-300"
          to="/parking-admin/schedules"
        >
          <FontAwesomeIcon :icon="faClock" class="mr-2 text-blue-500" />
          <span v-if="isSidebarOpen" class="leading-none">Horarios</span>
        </RouterLink>
        <RouterLink
          class="flex items-center flex-shrink-0 h-10 px-2 text-sm font-medium rounded hover:bg-gray-300"
          to="/parking-admin/parking-spaces"
        >
          <FontAwesomeIcon :icon="faParking" class="mr-2 text-blue-500" />
          <span v-if="isSidebarOpen" class="leading-none"
            >Espacios de parqueo</span
          >
        </RouterLink>
        <RouterLink
          class="flex items-center flex-shrink-0 h-10 px-2 text-sm font-medium rounded hover:bg-gray-300"
          to="/parking-admin/fees"
        >
          <FontAwesomeIcon :icon="faMoneyBill" class="mr-2 text-blue-500" />
          <span v-if="isSidebarOpen" class="leading-none">Tarifas</span>
        </RouterLink>
        <RouterLink
          class="flex items-center flex-shrink-0 h-10 px-2 text-sm font-medium rounded hover:bg-gray-300"
          to="/parking-admin/additional-services"
        >
          <FontAwesomeIcon :icon="faCarSide" class="mr-2 text-blue-500" />
          <span v-if="isSidebarOpen" class="leading-none"
    ...
    <div class="flex flex-col flex-grow">
      ...
      >
        <h1 class="text-lg font-medium">
          {{
            `Bienvenido: ${authStore.user?.name ?? 'Administrador de parqueadero'}`
          }}
        </h1>
      </div>
